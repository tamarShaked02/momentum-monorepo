...
if (bot) {
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      let customer = await prisma.customer.findFirst({ where: { telegramChatId: chatId, userId: businessUserId } });
      if (!customer) {
        customer = await prisma.customer.create({
          data: { userId: businessUserId, name: userName, telegramChatId: chatId },
        });
      }

      // Simple booking parser
      const bookingRegex = /book\s+(.+?)\s+(?:on\s+)?(.+?)\s+(?:at\s+)?(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)/i;
      ...
      if (match) {
        ...
        await ctx.reply(`✅ Appointment booked!\n📋 Service: ${service.trim()}\n📅 Date: ${appointmentDate.toLocaleDateString()}\n🕐 Time: ${appointmentDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}\n\nSee you then! 🎉`);
      } else {
        await ctx.reply('To book an appointment, send:\n"Book [service] on [day] at [time]"\n\nExample: "Book manicure on Friday at 2pm"');
      }
    } catch (error) {
