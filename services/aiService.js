...
const getMockResponse = (userDescription) => {
    ...
    if (userDescription.toLowerCase().includes('appointment') || userDescription.toLowerCase().includes('schedul')) {
        modules.push({ id: "scheduling", reason: "Detected appointment needs from your description." });
    }
    if (userDescription.toLowerCase().includes('sell') || userDescription.toLowerCase().includes('product') || userDescription.toLowerCase().includes('inventory')) {
        modules.push({ id: "inventory", reason: "Detected product/sales needs from your description." });
    }

    return {
        recommended_modules: modules,
        summary: "I've analyzed your request and enabled a mock configuration for testing purposes (AI Quota Exceeded or Mock Mode enabled)."
    };
};
